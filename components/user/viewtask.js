...
const ViewTask = ({ isViewTaskClosed, viewTaskCloseHandler, editTask, callbackUrl }) => {
    ...
    return (
        <div 
            className={`items-center absolute top-0 left-0 w-screen h-screen bg-task-ss-dark-blue-600 bg-opacity-50 z-20 ${isViewTaskClosed ? ' hidden ' : ' flex flex-col'}`} 
        >   
            <div className='bg-task-ss-white-100 w-[90%] md:w-[600px] h-auto rounded-lg mt-[5%] relative z-20'>
                ...
                <form method='POST' onSubmit={handleEditTask}>
                    <div className='flex flex-wrap items-center py-3 px-5'>
                        <h2 className='font-semibold text-lg mr-4'>View Task Details</h2>
                        {/* task type dropdown */}
                        <select 
                            value={taskType ?? ''}
